...
public class GuiEnchantmentTableHileb extends GuiContainer {
    /**
    ...
    private final ContainerEnchantmentTableHileb containerEnchantmentTableHileb;
    public int ticks;
    public float flip;
    public float oFlip;
    public float flipT;
    ...
    public GuiEnchantmentTableHileb(ContainerEnchantmentTableHileb containerEnchantmentTableHilebIn, InventoryPlayer inventory, World worldIn, IWorldNameable nameable) {
        super(containerEnchantmentTableHilebIn);
        this.playerInventory = inventory;
        this.containerEnchantmentTableHileb = containerEnchantmentTableHilebIn;
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        for (int k = 0; k < 3; ++k) {
            ...
            if (l >= 0 && i1 >= 0 && l < 108 && i1 < 19 && this.containerEnchantmentTableHileb.enchantItem(this.mc.player, k)) {
                this.mc.playerController.sendEnchantPacket(this.containerEnchantmentTableHileb.windowId, k);
            }
        }
    }
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        RenderHelper.disableStandardItemLighting();
        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
        EnchantmentNameParts.getInstance().reseedRandomGenerator((long) this.containerEnchantmentTableHileb.xpSeed);
        int k = this.containerEnchantmentTableHileb.getLapisAmount();
        ...
        for (int l = 0; l < 3; ++l) {
            ...
            this.zLevel = 0.0F;
            this.mc.getTextureManager().bindTexture(ENCHANTMENT_TABLE_GUI_TEXTURE);
            int k1 = this.containerEnchantmentTableHileb.enchantLevels[l];
            GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
            ...
            if (k1 == 0) {
                ...
                if (((k < l + 1 || this.mc.player.experienceLevel < k1) && !this.mc.player.capabilities.isCreativeMode) || this.containerEnchantmentTableHileb.enchantClue[l] == -1) // Forge: render buttons as disabled when enchantable but enchantability not met on lower levels
                {
                    this.drawTexturedModalRect(i1, j + 14 + 19 * l, 0, 185, 108, 19);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        int i = this.containerEnchantmentTableHileb.getLapisAmount();

        for (int j = 0; j < 3; ++j) {
            int k = this.containerEnchantmentTableHileb.enchantLevels[j];
            Enchantment enchantment = Enchantment.getEnchantmentByID(this.containerEnchantmentTableHileb.enchantClue[j]);
            int l = this.containerEnchantmentTableHileb.worldClue[j];
            int i1 = j + 1;
            ...
            if (this.isPointInRegion(60, 14 + 19 * j, 108, 17, mouseX, mouseY) && k > 0) {
                ...
                if (enchantment == null)
                    ...
                    if (this.mc.player.experienceLevel < k) {
                        list.add(TextFormatting.RED + I18n.format("container.enchant.level.requirement", this.containerEnchantmentTableHileb.enchantLevels[j]));
                    } else {
                        String s;
    ...
    public void tickBook() {
        ...
        for (int i = 0; i < 3; ++i) {
            if (this.containerEnchantmentTableHileb.enchantLevels[i] != 0) {
                flag = true;
            }
        }
