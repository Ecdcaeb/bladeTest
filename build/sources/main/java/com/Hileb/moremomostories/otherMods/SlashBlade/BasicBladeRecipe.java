...
public class BasicBladeRecipe extends ShapedOreRecipe {
    ...
    public BasicBladeRecipe(ResourceLocation group, ItemStack result, ItemStack inheritBlade, Object... recipe) {
     ...
     * @param recipe 合成
     */
    public BasicBladeRecipe(ResourceLocation group, ItemStack result,ItemStack inheritBlade, List<ItemStack> materialBlade, Object... recipe) {
        ...
        for(int idx = 0; idx < var1.getSizeInventory(); ++idx) {
            ...
            if (!curIs.isEmpty() && curIs.getItem() instanceof ItemSlashBlade && curIs.hasTagCompound()) {
                ...
                if (ItemSlashBladeNamed.CurrentItemName.get(oldTag).equals(ItemSlashBladeNamed.CurrentItemName.get(reqTag))) {
                    ...
                    newTag = ItemSlashBlade.getItemTagCompound(result);//开始nbt设置
                    ItemSlashBlade.KillCount.set(newTag, ItemSlashBlade.KillCount.get(oldTag));//继承基本数据
                    ...
                    if (oldTag.hasUniqueId("Owner")) {//继承主人
